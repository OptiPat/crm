    ...
    pub fn update_contact(&self, id: i64, contact: &NewContact) -> Result<Contact> {
        ...
        self.conn.execute(
            "UPDATE contacts SET 
                ...
                regime_matrimonial = ?19,
                revenus_annuels = ?20,
                charges_emprunts = ?21,
                objectifs_patrimoniaux = ?22,
                source_lead = ?23,
                profil_risque_sri = ?24,
                date_dernier_contact = ?25,
                date_prochain_suivi = ?26,
                date_dernier_contact_filleul = ?27,
                date_prochain_suivi_filleul = ?28,
                statut_suivi = ?29,
                registre = ?30,
                notes = ?31,
                role_foyer = ?32,
                role_famille = ?33,
                famille_regroupement_exclu = ?34,

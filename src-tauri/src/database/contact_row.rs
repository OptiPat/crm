...
pub fn map_contact_row(row: &Row<'_>) -> rusqlite::Result<Contact> {
    Ok(Contact {
        ...
        statut_occupation_logement: row.get(41)?,
        tranche_imposition: row.get(42)?,
        nombre_parts_fiscales: row.get(43)?,
        revenu_fiscal_reference: row.get(44)?,
        ir_net_a_payer: row.get(45)?,
        date_r1: row.get(46)?,
        type_invitation_filleul: row.get(47)?,
        date_invitation_filleul: row.get(48)?,
        date_inscription_filleul: row.get(49)?,
        presence_invitation_filleul: row.get(50)?,
        filleul_titre: row.get(51)?,
        filleul_qualification: row.get(52)?,
        filleul_volume: row.get(53)?,
        filleul_volume_manager: row.get(54)?,
    })
}

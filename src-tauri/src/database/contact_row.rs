...
pub fn map_contact_row(row: &Row<'_>) -> rusqlite::Result<Contact> {
    Ok(Contact {
        ...
        regime_matrimonial: row.get(21)?,
        revenus_annuels: row.get(22)?,
        charges_emprunts: row.get(23)?,
        objectifs_patrimoniaux: row.get(24)?,
        source_lead: row.get(25)?,
        profil_risque_sri: row.get(26)?,
        date_dernier_contact: row.get(27)?,
        date_prochain_suivi: row.get(28)?,
        date_dernier_contact_filleul: row.get(29)?,
        date_prochain_suivi_filleul: row.get(30)?,
        statut_suivi: row.get(31)?,
        registre: row.get(32)?,
        notes: row.get(33)?,
        famille_regroupement_exclu: row.get::<_, i64>(34)? != 0,
        created_at: row.get(35)?,
        updated_at: row.get(36)?,
        google_contact_resource_name: row.get(37)?,
        google_synced_at: row.get(38)?,
    })
}

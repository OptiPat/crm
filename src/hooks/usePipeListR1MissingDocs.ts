...
import { listPipeR1MissingDocsSummaries } from "@/lib/api/tauri-pipe-r1-checklist";
import {
...
export function usePipeListR1MissingDocs(
  enabled: boolean,
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const rows = await listPipeR1MissingDocsSummaries();
      const map: Record<number, string[]> = {};
      for (const row of rows) {
        const labels = mapPipeChecklistKeysToLabels(row.missing_item_keys, "R1", templates);
        if (labels.length > 0) {
  ...
  useEffect(() => {
    ...
    const onChecklistChanged = (detail?: { pipeId: number; missingItemKeys: string[] }) => {
      ...
      setMissingByPipeId((prev) => {
        const next = { ...prev };
        const labels = mapPipeChecklistKeysToLabels(detail.missingItemKeys, "R1", templates);
        if (labels.length === 0) {
    ...
    const unsubChecklist = subscribePipeR1ChecklistChanged(onChecklistChanged);

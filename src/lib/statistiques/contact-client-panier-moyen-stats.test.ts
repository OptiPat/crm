...
describe("contact-client-panier-moyen-stats", () => {
  ...
  it("compare le montant souscrit perso au panier moyen global", () => {
    ...
    expect(contactMontantSouscritAvecMoiEuros(contacts[0]!, indexes, activeClientCountByFoyer)).toBe(

      20_000

    );

    expect(contactMontantSouscritAvecMoiEuros(contacts[1]!, indexes, activeClientCountByFoyer)).toBe(
    ...
    expect(stats.abovePercent).toBe(50);
  ...
  it("répartit le commun foyer entre clients actifs du foyer", () => {
    ...
    const contacts = [
      ...
      contact({ id: 2, foyer_id: 10 }),

    ];

    const investissements = [

      inv({ id: 1, foyer_id: 10, montant_initial: 10_000_000 }),
    ...
    const panierMoyen = 50_000;

    const stats = computeClientAbovePanierMoyenStats(contacts, investissements, panierMoyen);

    expect(stats.aboveCount).toBe(0);

    expect(stats.atOrBelowContactIds).toEqual([1, 2]);
  ...
  it("répartit les placements foyer rattachés à un déclarant entre conjoints", () => {
    ...
    const panierMoyen = 55_000;
    const stats = computeClientAbovePanierMoyenStats(contacts, investissements, panierMoyen);
    expect(stats.aboveCount).toBe(2);
    expect(stats.aboveContactIds).toEqual([1, 2]);
  });

  it("additionne perso hors foyer et part du pool foyer", () => {
    ...
    const investissements = [
      ...
      inv({ id: 1, contact_id: 1, montant_initial: 3_000_000 }),

      inv({ id: 2, foyer_id: 10, montant_initial: 10_000_000 }),
    ...
    expect(stats.aboveContactIds).toEqual([1]);

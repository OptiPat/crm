...
import dupontPassifsFixture from "./fixtures/rio-solo-dupont-passifs-2026.txt?raw";
import {
...
describe("immo-credits — Dupont solo passifs 2026 (PDF réel)", () => {
  it("parse les crédits avec bien dans la désignation Passifs", () => {
    const credits = parseStelliumPassifsMortgageCredits(dupontPassifsFixture);
...
Crédit immobilier - Amortissable - Primo MTP\tJean DUPONT\t9 454 €\t166 183 €\t10/11/2046
Crédit immobilier - Amortissable - Pinel sète\tJean DUPONT\t8 306 €\t141 267 €\t24/12/2046
Revenus et charges`;
...
Revenus et charges`;
  ...
  it("fusionne les lignes Passifs coupées comme le PDF Dupont 16/06/2026", () => {
...
Revenus et charges`;
  ...
  it("associe chaque crédit au bon bien immobilier", () => {
    const data = parseStelliumRio(dupontPassifsFixture);
  ...
  it("n'écrase pas la RP avec le crédit Pinel", () => {
    ...
    enrichBiensImmobiliersWithCredits(dupontPassifsFixture, biens);

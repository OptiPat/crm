...
import { ClientPreviewOverlayPortalProvider } from "./client-preview-overlay";
...
export function ClientPreviewDeviceFrame({
  ...
  const [overlayPortalEl, setOverlayPortalEl] = useState<HTMLElement | null>(null);
  const overlayPortalRef = useCallback((node: HTMLDivElement | null) => {
    setOverlayPortalEl(node);
  }, []);
    ...
    <ClientPreviewOverlayPortalProvider portalEl={overlayPortalEl}>
      ...
      >
        ...
        >
          ...
          {/*
            Écran visible : hauteur bornée. Le scroll vit dedans ; les modales
            se portent dans la couche absolute inset-0 (pas dans le flux scrollé).
          */}
          <div
            className={cn(
              "relative mx-0.5 mb-0.5 flex flex-col bg-[var(--cp-bg)]",
              isMobile
                ? "max-h-[min(78vh,820px)] min-h-[640px] rounded-[2.2rem]"

...
import { FICHE_CONSEIL_ARBITRAGE_REDACTION_LABELS } from "@/lib/pdf/arbitrage-fiche-conseil/arbitrage-redaction-labels";
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId === "new") {
        ...
        await updateFicheConseilRedactionPreset(editingId, {
          nom,
          motif: draft.motif.trim(),
          supports_desinvestis: draft.supports_desinvestis.trim(),
          supports_investis: draft.supports_investis.trim(),
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          Textes réutilisables pour les fiches conseil arbitrage (motif et supports).
        </p>
      ...
      {editingId != null ? (
        <div className="space-y-3 rounded-lg border p-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="preset-motif">{FICHE_CONSEIL_ARBITRAGE_REDACTION_LABELS.motif}</Label>
            <Textarea
              id="preset-motif"
              value={draft.motif}

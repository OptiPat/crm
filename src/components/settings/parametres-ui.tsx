...
import type { SettingsNavGroup } from "@/lib/settings/parametres-nav";

export type { SettingsNavItem } from "@/lib/settings/parametres-nav";
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
        <aside className="lg:w-[220px] shrink-0">
          <nav
            className="flex gap-2 overflow-x-auto pb-1 lg:flex-col lg:overflow-visible lg:pb-0 lg:sticky lg:top-0 lg:space-y-4"
            aria-label="Sections des paramètres"
          >
            {navGroups.map((group) => (
              <div key={group.id} className="lg:space-y-1">
                <p className="hidden lg:block px-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/80">
                  {group.label}
                </p>
...
export function StatusTile({
  ...
  return (
    <Wrapper
      ...
      onClick={onClick}
      className={cn(
        "flex items-center gap-2.5 rounded-lg border border-border/80 bg-card px-2.5 py-2 text-left shadow-sm transition-shadow",
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm font-semibold text-primary leading-tight mt-0.5 truncate">{value}</p>
        <p className="text-[11px] text-muted-foreground leading-snug mt-0.5 line-clamp-1">{subtitle}</p>
      </div>
    </Wrapper>
  );
}
